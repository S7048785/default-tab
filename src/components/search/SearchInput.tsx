...
export default function SearchInput() {
	...
	return (
		<>
			<div className={"relative w-[40rem] flex flex-col"}>
				<motion.form
					onSubmit={onSubmit}
					...
					className={"mx-auto"}
				>
					<div className={"h-14 rounded-full dark:bg-gray-700 bg-white px-6 flex duration-300 shadow-lg items-center w-full"}>
						<input id="select"
							ref={inputRef}
							value={searchState}
				...
				{/*下拉菜单 历史记录*/}
				{
					isShow && <motion.div
						...
						className="w-full absolute rounded-lg -z-10 dark:bg-white/10 bg-white/40 backdrop-blur-lg top-[4.5rem] ">
						{/* 玻璃背景 */}
							...
							<div className={"mx-2"}>
								{
									historyStore.historySearchData.map((item, index) => (
										...
										}} key={item} className={"history my-2 p-2 py-3 rounded-lg flex justify-between cursor-pointer hover:bg-gray-500/10 dark:hover:bg-white/10"}>
